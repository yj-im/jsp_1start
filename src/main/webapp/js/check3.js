...
// check2.js 코드를 for문으로 변경해보기.
function checkValues(){
	let isSubmit = true
	// css 선택자를 이용하여 검사해야할 요소들(All)을 가져오기
	const test=document.querySelectorAll('div.input-group > input, div.input-group > select');
	...
	reverse.forEach( element => {
		...
		if(element.value==='' && element.name !='email'){
			...
			element.nextElementSibling='필수 입력 입니다.'
			element.focus()
			isSubmit=false	
		}
	})
